        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="form-group required row mb-2">
                    ...
                    <div class="col-sm-10">
                        <input type="text" class="form-control form-control-sm" id="berita_judul" name="berita_judul"
                            value="{{ isset($data->berita_judul) ? $data->berita_judul : '' }}" />
                ...
                <div class="form-group required row mb-2">
                    ...
                    <div class="col-sm-10 mt-2">
                        <div class="icheck-success d-inline mr-3">
                            <input type="radio" id="radioActive" name="berita_status" value="1"
                                <?php echo isset($data->berita_status) ? ($data->berita_status == 1 ? 'checked' : '') : 'checked'; ?>>
                            <label for="radioActive">Publish </label>
                        </div>
                        <div class="icheck-danger d-inline mr-3">
                            <input type="radio" id="radioFailed" name="berita_status" value="0"
                                <?php echo isset($data->berita_status) ? ($data->berita_status == 0 ? 'checked' : '') : ''; ?>>
                            ...
                            @if ($is_edit)
                                @foreach ($data->getDokumen as $r)
                                    <tr class="pld_{{ $r->berita_dokumen_id }}">
                                        <td>{{ $r->file_name }}</td>
                                        <td class="align-top">
                                            <a class="pop_preview" href="#" data-src="{{ asset($r->file_url) }}"
                                                data-type="{{ $r->file_ext }}"><em>preview</em> {!! $r->file_ext == 'pdf'
                                                    ? '<span class="badge bg-fuchsia">pdf</span>'
                                                    : '<span class="badge bg-purple">image</span>' !!}
                                            </a>
                                        </td>
                                        <td>
                                            <a href="#" data-block="body"
                                                data-url="{{ url($page->url . '/' . $r->berita_dokumen_id . '/delete') }}"
                                                class="ajax_modal_confirm btn btn-sm btn-danger tooltips text-light"
                                ...
                                <td class="align-top">
                                    <div class="form-control-sm custom-file">
                                        <input type="file" class="form-control-sm custom-file-input" data-target="0"
                                            id="berita_doc_0" name="berita_dokumen[0]" data-rule-filesize="1"
                                            data-rule-accept="application/pdf,image/png,image/jpg"
                                            accept="application/pdf,image/png,image/jpg" />
                                        <label class="form-control-sm custom-file-label file_label_0"
                                            for="berita_doc_0">Choose file</label>
                                    </div>
                                </td>
                                <td class="align-top"><input type="text"
                                        class="form-control form-control-sm file_name file_name_0" name="file_name[0]"
                                        value=""></td>
                                <td class="align-top"><button type="button" class="btn btn-sm btn-success"
                                        onclick="addRow(this)"><i class="fa fa-plus"></i></button></td>
                            </tr>
                        </table>
                    </div>
                </div> --}}
...
<script>
    function addRow(th) {
        ...
        let n = $(th).closest('tr').clone(true).html(function(i, t) {
            return t.replace('btn-success', 'btn-warning').replace('fa-plus', 'fa-trash').replace(
                'addRow(this)', 'delRow(this)').replaceAll('0', it);
        });
        n.find('span').remove();
    ...
    var loadFile = function(event) {
        $('input.custom-file-input').on('change', function() {
            $('.file_name_' + $(this).data('target')).val($('.file_label_' + $(this).data('target'))
                .text());
        });
    };
    ...
    $(document).ready(function() {
        ...
        $('.pop_preview').on('click', function() {
            if ($(this).data('type') == 'pdf') {
                $('#preview_doc').html('<object data="' + $(this).data('src') +
                    '" type="application/pdf" width="100%" height="580px" /><p><a href="' + $(this)
                    .data('src') + '">Download</a> berkas pdf.</p> </object>');
            } else {
